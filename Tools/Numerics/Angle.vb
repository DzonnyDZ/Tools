...
#Region "ToString"
        ...
        ''' <item>      <term>D, H     </term><description><see cref="TotalDegrees"/>              </description><description>Yes - minimum number of digits left from decimal point</description><description>Number - number of decimal places (as required if not present)                                           </description><description>If you want to exclude days use H instead.                                                                                                                                                          </description></item>
        ...
        ''' <item>      <term>E        </term><description><see cref="Rotations"/>                 </description><description>Yes - minimum number of digits left from decimal point</description><description>Number - number of decimal places (max <see cref="NumberFormatInfo.NumberDecimalDigits"/> if not present)</description><description>                                                                                                                                                                                                    </description></item>
        ...
        ''' <item>      <term>p, π     </term><description>π-radians (<see cref="ToRadians"/> / π )</description><description>Yes - minimum number of digits left from decimal point</description><description>Number - number of decimal places (max <see cref="NumberFormatInfo.NumberDecimalDigits"/> if not present)</description><description>Value of <see cref="ToRadians"/> (angle in radians) divided by <see cref="π"/> - because radian values are often given as mupltiples of π (pi)                                                      </description></item>
        ''' <item>      <term>R        </term><description><see cref="ToRadians"/>                 </description><description>Yes - minimum number of digits left from decimal point</description><description>Number - number of decimal places (max <see cref="NumberFormatInfo.NumberDecimalDigits"/> if not present)</description><description>                                                                                                                                                                                                    </description></item>
        ...
        ''' <item>      <term>Z        </term><description><see cref="ToGradians"/>                </description><description>Yes - minimum number of digits left from decimal point</description><description>Number - number of decimal places (max <see cref="NumberFormatInfo.NumberDecimalDigits"/> if not present)</description><description>                                                                                                                                                                                                    </description></item>
        ''' </list>
        ...
        ''' Property name is generally case-insensitive with exception of optional SF prefix. If the case-sensitive SF prefix is included at the begining of property name formatting string is passed to <see cref="System.String.Format"/> and it is expected to contain placeholder {0} to be replaced with formatted property value. Otherwise formatting string is passed to <see cref="IFormattable.ToString"/> (all properties return <see cref="IFormattable"/>).
        ...
        ''' <item><term>RestSeconds</term><description><see cref="Decimal"/></description><description><see cref="TotalSeconds"/> excluding minutes and degrees. Same as <c>Seconds</c>.</description></item>
                ...
                Select Case state
                    Case FState.Start, FState.Normal
                        ...
                        Select Case ch
                            ...
                            Case "D"c, "H"c  'D degrees, decimal                                                             no dec places          as required    min digits        
                                value = Math.Abs(normalizedValue.TotalDegrees) : state = FState.SpecifierDouble : specifier = ch : has_d = True
                            Case "e"c  'o slope (absolute)                                                             no decimal places      as required        min digits    
                                value = Math.Abs(normalizedValue.ToSlope) / 100.0# : specifier = ch : state = FState.SpecifierDouble
                            Case "E"c  'E Rotations value                                                              no decimal places      max 4          min digits        
                                value = Math.Abs(normalizedValue.Rotations) : specifier = ch : state = FState.SpecifierDouble : rightPlaces = -ninfo.NumberDecimalDigits
                            Case "f"c  'f second fraction                                                              no decimal places max  as required    min digits        
                                value = (normalizedValue.Seconds - Math.Floor(normalizedValue.Seconds)) * 10@
                                specifier = ch : state = FState.SpecifierF : leftPlaces = 1
                            Case "h"c  'h degrees, whole, excl. days                                                      not allowed                        min digits        
                                value = CInt(Math.Floor(Math.Abs(normalizedValue.TotalDegrees) - (Math.Abs(normalizedValue.TotalDegrees) \ 24.0#) * 24.0#))
                            ...
                            Case "l"c  'l permile slope                                                                no decimal places      as required       min digits     
                                value = Math.Abs(normalizedValue.ToSlope) * 10.0# : specifier = ch : state = FState.SpecifierDouble
                            Case "L"c  'l percent slope                                                                no decimal places      as required       min digits     
                                value = Math.Abs(normalizedValue.ToSlope) : specifier = ch : state = FState.SpecifierDouble
                            ...
                            Case "M"c  'M minutes, decimal (excl degrees if d/D or h/H present)                                 no decimal places      as required   min digits
                                value = If(has_d, Math.Abs(normalizedValue.TotalMinutes) - CDbl(normalizedValue.Degrees) * 60.0#, Math.Abs(normalizedValue.TotalMinutes))
                            ...
                            Case "π"c, "p"c  'π-radians value                                                                no decimal places      max 4          min digits  
                                value = Math.Abs(normalizedValue.ToRadians) / π : specifier = ch : state = FState.SpecifierDouble : rightPlaces = -ninfo.NumberDecimalDigits
                            Case "R"c  'R radians value                                                                no decimal places      max 4          min digits        
                                value = Math.Abs(normalizedValue.ToRadians) : specifier = ch : state = FState.SpecifierDouble : rightPlaces = -ninfo.NumberDecimalDigits
                            ...
                            Case "y"c  'y days, whole                                                                  not allowed                           min digits        
                                value = Math.Floor(Math.Abs(normalizedValue.TotalDegrees) / 24) : state = FState.SpecifierInt : specifier = ch : has_y = True
                            Case "Y"c  'Y days, decimal                                                                no decimal places      as required    min digits        
                                value = Math.Abs(normalizedValue.TotalDegrees) / 24.0# : state = FState.SpecifierDouble : specifier = ch : has_y = True
                            Case "Z"c  'Z grads value                                                                  no decimal places      max 4          min digits        
                                value = Math.Abs(normalizedValue.ToGradians) : specifier = ch : state = FState.SpecifierDouble : rightPlaces = -ninfo.NumberDecimalDigits
                    ...
                    Case FState.SpecifierF 'Specifier for sub-second values
                        Select Case ch
                            Case "f"c : leftPlaces += 1 : value = DirectCast(value, Decimal) * 10@
...
[end]:
            Select Case state
                ...
                Case FState.Plus '+ and whitespaces
                    Dim oldBuilder = builders.Pop
                    If normalizedValue.ToSlope < 0 Then builders.Peek.Append(ninfo.NegativeSign) Else builders.Peek.Append(ninfo.PositiveSign)
                    builders.Peek.Append(oldBuilder.ToString)
                    state = FState.Normal : GoTo [end]
                Case Else : Throw New ApplicationException("Unknown FSA state {0}".f(state)) 'Should never happen
            End Select
        ...
        Private Shared Sub AppendCustom(name As String, builder As MeaningfulStringbuilder, format As String, formatProvider As IFormatProvider, normalizedValue As Angle)
            ...
            Select Case name.ToLowerInvariant
                Case "days" : value = CInt(normalizedValue.TotalDegrees \ 24@)
                ...
                Case "resthours" : value = Math.Abs(normalizedValue.TotalDegrees) - Math.Floor(Math.Abs(normalizedValue.TotalDegrees) / 24@) * 24@
                Case "restminutes" : value = normalizedValue.TotalMinutes - normalizedValue.Degrees * 60@
                Case "restseconds" : value = normalizedValue.Seconds
        ...
        Private Shared Sub AppendValueF(value As Double, maxDecPlaces%, minDecPlaces%, builder As MeaningfulStringbuilder, formatProvider As IFormatProvider)
            ...
            builder.Meaningful = builder.Meaningful OrElse mmfl
        End Sub
        ...
        Private Shared Sub AppendValue(value As IFormattable, leftPlaces As Integer?, rightPlaces As Integer?, builder As MeaningfulStringbuilder, useThousandSeparator As Boolean, formatProvider As IFormatProvider)
            ...
            builder.Meaningful = builder.Meaningful OrElse If(
                TypeOf value Is Integer, DirectCast(value, Integer) <> 0,
                Math.Round(DirectCast(value, Double), If(rightPlaces, 15)) <> 0.0#
            )
        End Sub
