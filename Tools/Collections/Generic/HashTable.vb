...
#End Region
        ...
        Public Class EqualityComparerAdapter : Implements IEqualityComparer(Of TKey), IEqualityComparer

            ''' <summary><see cref="IEqualityComparer(Of T)"/> to be adapted</summary>
            Private TSEqComp As IEqualityComparer(Of TKey)
            ''' <summary>CTor</summary>
            ''' <param name="AdaptThis"><see cref="IEqualityComparer(Of T)"/> to be adapted</param>
            Public Sub New(ByVal AdaptThis As IEqualityComparer(Of TKey))
                TSEqComp = AdaptThis
            End Sub
            ...
            Public Overloads Function Equals(ByVal x As TKey, ByVal y As TKey) As Boolean Implements System.Collections.Generic.IEqualityComparer(Of TKey).Equals
                Return TSEqComp.Equals(x, y)
            End Function
            ''' <summary>Returns a hash code for the specified object.</summary>
            ''' <param name="obj">The <see cref="TKey"/> for which a hash code is to be returned.</param>
            ''' <returns>A hash code for the specified object.</returns>
            ''' <exception cref="System.ArgumentNullException">The type of obj is a reference type and obj is null.</exception>
            Public Overloads Function GetHashCode(ByVal obj As TKey) As Integer Implements System.Collections.Generic.IEqualityComparer(Of TKey).GetHashCode
            ...
            Public Overloads Function Equals(ByVal x As Object, ByVal y As Object) As Boolean Implements System.Collections.IEqualityComparer.Equals
                If TypeOf x Is TKey AndAlso TypeOf y Is TKey Then
                    Return TSEqComp.Equals(x, y)
                Else
                    Return y.Equals(y)
                End If
...
#End Region
    ...
    <Author("Đonny", "dzonny.dz@gmail.com"), Version(1, 0, GetType(HashTable(Of Long)), LastChange:="12/25/2006")> _
    Public Class HashTable(Of T)
        Implements ICollection(Of T)
        ''' <summary><see cref="Hashtable"/> internally used by this instance</summary>
        Protected Internal As HashTable
#Region "CTors"
        ...
        Public Sub New(Optional ByVal EqualityComparer As IEqualityComparer(Of T) = Nothing)
            ...
            Else
                Internal = New HashTable(New HashTable(Of T, T).EqualityComparerAdapter(EqualityComparer))
            End If
        End Sub
        ...
        Public Sub New(ByVal Capacity As Integer, Optional ByVal EqualityComparer As IEqualityComparer(Of T) = Nothing)
            If EqualityComparer Is Nothing Then
                Internal = New HashTable(Capacity)
            Else
                Internal = New HashTable(Capacity, New HashTable(Of T, T).EqualityComparerAdapter(EqualityComparer))
            End If
        End Sub
        ...
        Public Sub New(ByVal Capacity As Integer, ByVal LoadFactor As Single, Optional ByVal EqualityComparer As IEqualityComparer(Of T) = Nothing)
            If EqualityComparer Is Nothing Then
                Internal = New HashTable(Capacity, LoadFactor)
            Else
                Internal = New HashTable(Capacity, LoadFactor, New HashTable(Of T, T).EqualityComparerAdapter(EqualityComparer))
            End If
        End Sub
#End Region
        ...
        Public Function Remove(ByVal item As T) As Boolean Implements System.Collections.Generic.ICollection(Of T).Remove
            If Internal.ContainsKey(item) Then
                Try
                    ...
                    Return Not Internal.Contains(item)
                Catch
                    Return False
                End Try
            Else
        ...
        Public Function GetEnumerator() As System.Collections.Generic.IEnumerator(Of T) Implements System.Collections.Generic.IEnumerable(Of T).GetEnumerator
            Return New ObjectToGenericEnumeratorAdapter(Internal.GetEnumerator)
        End Function
        ...
        ''' <summary>Adapts type-unsafe <see cref="IEnumerator"/> into type-sefe <see cref="IEnumerator(Of T)"/></summary>
        ''' <remarks>Exception can occure when enumerating through collection that contains values of different types</remarks>
        Public Class ObjectToGenericEnumeratorAdapter : Implements IEnumerator(Of T)
            ''' <summary><see cref="IEnumerator"/> to be adapted</summary>
            Private AdaptThis As IEnumerator
            ''' <summary>CTor</summary>
