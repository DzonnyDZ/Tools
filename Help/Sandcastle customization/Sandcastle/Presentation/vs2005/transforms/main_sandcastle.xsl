...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
    ...
    <xsl:template name="body">
        ...
        <!--Added by Đonny-->
        <xsl:apply-templates select="/document/comments/author"/>
        <xsl:call-template name="VersionHistory"/>
        <!-- syntax -->
    ...
    <xsl:template name="VersionHistory">
        <xsl:if test="count(/document/comments/version) > 0">
            <xsl:call-template name="section">
                <xsl:with-param name="toggleSwitch" select="'versionhistory'"/>
                <xsl:with-param name="title" select="'Vertion history'"/>
                <xsl:with-param name="content">
                    <dl>
                        <xsl:variable name="vh">
                            <xsl:for-each select="/document/comments/version">
                                <xsl:sort select="substring-before(@version,'.')" data-type="number"/>
                                <xsl:sort select="substring-before(substring-after(@version,'.'),'.')" data-type="number"/>
                                <xsl:sort select="substring-before(substring-after(substring-after(@version,'.'),'.'),'.')" data-type="number"/>
                                <xsl:sort select="substring-after(substring-after(substring-after(@version,'.'),'.'),'.')" data-type="number"/>
                                <xsl:sort select="boolean(@stage)" data-type="number" order="descending"/>
                                <xsl:apply-templates select="." />
                                <xsl:copy-of select="."/>
                            </xsl:for-each>
                        </xsl:variable>
                        <xsl:call-template name="VersionHistory2">
                            <xsl:with-param name="vh" select="msxsl:node-set($vh)"/>
                        </xsl:call-template>
                    </dl>
                </xsl:with-param>
            </xsl:call-template>            
    ...
    <xsl:template name="VersionHistory2">
        <xsl:param name="vh"/>
        <xsl:for-each select="$vh/version">
            <xsl:variable name="current" select="."/>
            <xsl:variable name="current-position" select="position()"/>
            <xsl:if test="$current-position = 1 or not($vh/version[$current-position - 1]/@version = $current/@version)">
                <dt>
                    <xsl:value-of select="@version"/>
                    <xsl:text xml:space="preserve"> </xsl:text>
                    <xsl:if test="@stage">(<xsl:choose>
                        <xsl:when test="@stage='RC'">Release Candidate</xsl:when>
                        <xsl:otherwise>
                ...
                <dd>
                    <xsl:choose>
                        <xsl:when test="$current-position = count($vh/version) or not($vh/version[$current-position + 1]/@version = $current/@version)">
                            <xsl:apply-templates/> 
                        </xsl:when>
                        <xsl:otherwise>
                            <ul>
                                <xsl:for-each select="$vh/version[@version = $current/@version]">
                                    <li>
                                        <xsl:apply-templates/>
                                    </li>
                                </xsl:for-each>
