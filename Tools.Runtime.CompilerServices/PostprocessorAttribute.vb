...
Namespace RuntimeT.CompilerServicesT
    ...
    ''' </summary>
    ''' <remarks>It's recommended that your post-processing class derives from <see cref="PostprocessingAttribute"/>.</remarks>
    ...
    Public NotInheritable Class PostprocessorAttribute
        ...
        ''' 3rd (if present) must <see cref="Type.IsAssignableFrom">be assignable from</see> <see cref="IPostprocessorContext"/>.
        ...
        '''     <see cref="System.Type.AssemblyQualifiedName"/> and <see cref="System.Type.[GetType]()"/>.
        ''' </param>
        ''' <param name="method">
        ''' Name of postprocessing method. The method must be member of type <paramref name="type"/>, it must be public, static and it must have 2 or 3 parameters.
        ''' 1st one must be of type <see cref="T:Mono.Cecil.ICustomAttributeProvider"/>.
        ''' 2nd one should accept the type this attribute as applied on.
        ''' 3rd (if present) must <see cref="System.Type.IsAssignableFrom">be assignable from</see> <see cref="IPostprocessorContext"/>.
        ''' If these conditions are not satisfied postprocessing will fail.
        ''' </param>
        ''' <exception cref="ArgumentNullException">Any argument is null</exception>
        ''' <seelaso cref="T:Tools.RuntimeT.CompilerServicesT.PostprocessorWithContext`1"/>
        ...
        ''' <returns>
        '''     If <see cref="System.Type"/> was passed to the constructor returns that type. Otherwise returns type obtained using
        '''     <see cref="System.Type.[GetType]()"/> from <see cref="TypeName"/> (which may return null if the type cannot be found).
        ''' </returns>
