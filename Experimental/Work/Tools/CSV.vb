...
#Region "Properties"
    ...
    Private WithEvents _Rows As New ListWithEvents(Of ListInternal)
    ...
    Public ReadOnly Property RowsInternal() As ListWithEvents(Of ListInternal)
        Get
            Return _Rows
        End Get
    End Property
...
#End Region
    ...
    Public Overridable Sub Load(ByVal r As IO.TextReader, Optional ByVal EmptyLines As EmptyLines = EmptyLines.IgnoreOnlyOne)
        ...
        If (EmptyLines And CSV.EmptyLines.IgnoreAtBeginning) = CSV.EmptyLines.IgnoreAtBeginning Then
            ...
            Loop
            If i > 0 Then
                For ò As Integer = i - 1 To 0 Step -1
                    RowsInternal.RemoveAt(ò)
                Next
            End If
        End If
        If (EmptyLines And CSV.EmptyLines.IgnoreAtEnd) = CSV.EmptyLines.IgnoreAtEnd Then
            ...
            Loop
            If i > 0 Then
                For ò As Integer = i - 1 To RowsInternal.Count - i Step -1
                    RowsInternal.RemoveAt(ò)
                Next
            End If
        End If
    ...
    Public ReadOnly Property MaxColumns() As UInteger
        Get
            If MaxColsCache >= 0 Then Return MaxColsCache
            Dim Max As UInteger = 0
            For Each Row As ListWithEvents(Of String) In RowsInternal
    ...
    Public ReadOnly Property MinColumns() As UInteger
        Get
            If MinColsCache >= 0 Then Return MinColsCache
...
#Region "Type descriptor" 'Implementuje type descriptor pro ListInternal, aby se tváøil, že má vlastnosti èíslované podle svých položek
    ...
    Public Class ListInternal : Inherits ListWithEvents(Of String)
    ...
    Private Class ListTypeDescriptor : Inherits CustomTypeDescriptorBase(Of ListInternal)
        ...
        Public Sub New(ByVal Instance As ListWithEvents(Of String), ByVal MaxCols As Integer)
            MyBase.New(Instance)
            Me.MaxCols = MaxCols
        End Sub
...
#End Region
    ...
    End Class
    ''' <summary>Enumerátor položek v øádku CSV</summary>
    Private Class RowEnumerator : Implements IEnumerator(Of String)
        Private CSV As CSV
        Private row As ListInternal
        Private index As Integer = -1
        Private ReadOnly Property CSVMaxColumns%()
            Get
                Return CSV.MaxColumns
            End Get
        End Property
        Public Sub New(ByVal CSV As CSV, ByVal row As ListInternal)
            Me.CSV = CSV
        ...
        Public ReadOnly Property Current() As String Implements System.Collections.Generic.IEnumerator(Of String).Current
            Get
                ...
                ElseIf index < CSVMaxColumns% Then : Return ""
...
#Region "Events"
    Private Sub Rows_Added(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal).ItemIndexEventArgs) Handles _Rows.Added
        AddHandler e.Item.Added, AddressOf Item_Added
        AddHandler e.Item.Removed, AddressOf Item_Removed
        AddHandler e.Item.Cleared, AddressOf Item_Cleared
        If MaxColsCache >= 0 AndAlso e.Item.Count > MaxColsCache Then MaxColsCache = e.Item.Count
        If MinColsCache >= 0 AndAlso e.Item.Count < MinColsCache Then MinColsCache = e.Item.Count
    End Sub

    Private Sub Rows_Cleared(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal).ItemsEventArgs) Handles _Rows.Cleared
        For Each row As ListInternal In e.Items
            RemoveHandler row.Added, AddressOf Item_Added
            RemoveHandler row.Removed, AddressOf Item_Removed
            RemoveHandler row.Cleared, AddressOf Item_Cleared
        Next
        MaxColsCache = 0
        MinColsCache = 0
    End Sub
    ''' <summary>Nacachovaná hodnota <see cref="MinColumns"/>; -1 pokud není platná</summary>
    Private MinColsCache% = -1
    ''' <summary>Nacachovaná hodnota <see cref="MaxColumns"/>; -1 pokud není platná</summary>
    Private MaxColsCache% = -1

    Private Sub Rows_ItemChanged(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal).OldNewItemEventArgs) Handles _Rows.ItemChanged
        RemoveHandler e.OldItem.Added, AddressOf Item_Added
        RemoveHandler e.OldItem.Removed, AddressOf Item_Removed
        RemoveHandler e.OldItem.Cleared, AddressOf Item_Cleared
        If e.OldItem.Count = MinColsCache Then MinColsCache = -1
        If e.OldItem.Count = MaxColsCache Then MaxColsCache = -1
        AddHandler e.Item.Added, AddressOf Item_Added
        AddHandler e.Item.Removed, AddressOf Item_Removed
        AddHandler e.Item.Cleared, AddressOf Item_Cleared
        If MaxColsCache >= 0 AndAlso e.Item.Count > MaxColsCache Then MaxColsCache = e.Item.Count
    ...
    Private Sub Rows_Removed(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of ListInternal).ItemIndexEventArgs) Handles _Rows.Removed
        RemoveHandler e.Item.Added, AddressOf Item_Added
        RemoveHandler e.Item.Removed, AddressOf Item_Removed
        RemoveHandler e.Item.Cleared, AddressOf Item_Cleared
        If e.Item.Count = MinColsCache Then MinColsCache = -1
        If e.Item.Count = MaxColsCache Then MaxColsCache = -1
    End Sub
    '
    Private Sub Item_Added(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of String), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of String).ItemIndexEventArgs)
        If sender.Count - 1 = MinColsCache Then MinColsCache = -1
        If MaxColsCache >= 0 AndAlso sender.Count > MaxColsCache Then MaxColsCache = sender.Count
    End Sub

    Private Sub Item_Cleared(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of String), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of String).ItemsEventArgs)
        If e.Items.Length = MaxColsCache Then MaxColsCache = -1
        If MinColsCache >= 0 AndAlso MinColsCache > 0 Then MinColsCache = 0
    End Sub

    Private Sub Item_Removed(ByVal sender As Tools.CollectionsT.GenericT.ListWithEvents(Of String), ByVal e As Tools.CollectionsT.GenericT.ListWithEvents(Of String).ItemIndexEventArgs)
        If sender.Count + 1 = MaxColsCache Then MaxColsCache = -1
        If MinColsCache >= 0 AndAlso MinColsCache > sender.Count Then MinColsCache = sender.Count
    End Sub
#End Region
End Class
